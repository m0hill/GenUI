...
void test("preferences.save requires approval and replays one completed result", async () => {
  ...
  assert.deepEqual(await executeGeneratedUiAction(call, () => undefined), {
    ok: false,
    error: {
  ...
  assert.deepEqual(await executeGeneratedUiAction(call, () => true), {
    ok: true,
    value: { preference: "Mountain escape" },
  })
  assert.deepEqual(await executeGeneratedUiAction(call, () => false), {
    ok: true,
    value: { preference: "Mountain escape" },
  })

  const deniedCall = { ...call, callId: "save-denied" }
  assert.deepEqual(await executeGeneratedUiAction(deniedCall, () => false), {
    ok: false,
